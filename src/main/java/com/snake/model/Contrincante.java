...
public class Contrincante extends Thread {
	...
	private static int REFRESH_SPEED_MILIS = 300;
	private static int REFRESH_SPEED_NANOS = 0;
	...
	public Contrincante(GamePanel gamePanel, boolean active, int x, int y, Color color, ReentrantLock lock) {
		super();
	...
	public void update() {
		...
		while (active) {
			...
			// Busca la posición de la bolita
			findBall();

			// Actualiza la matrix de ubicaciones
			updateMatrix();

			// Determina el mejor movimiento y realizalo
			moveToBall();
		}
	}
	...
	// Calcula la distancia de ese punto a la bolita, tomando en cuenta como puntos
	// especiales
	// el cuerpo de la serpiente y la bolita misma
	...
	// Realiza el movimiento que le indican
	...
	// Revisamos uno o dos movimientos a la derecha para ver si es posible ese
	// Movimiento y es el "mejor" movimiento en la vecindad inmediata, selecciona
	// el menor costo
	private int decideMove() {
		...
		if (i + 1 < pathMatrix.length && pathMatrix[i + 1][j] < min && lastMove != KeyEvent.VK_LEFT) {
		...
		if (j - 1 > 0 && pathMatrix[i][j - 1] < min && lastMove != KeyEvent.VK_DOWN) {
		...
		if (j + 1 < pathMatrix[0].length && pathMatrix[i][j + 1] < min && lastMove != KeyEvent.VK_UP) {
